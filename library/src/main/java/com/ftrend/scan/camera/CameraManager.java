package com.ftrend.scan.camera;
...
import com.ftrend.scan.camera.focus.FocusManager;
import com.ftrend.scan.camera.open.OpenCamera;
import com.ftrend.scan.camera.open.OpenCameraInterface;
import com.ftrend.scan.core.BarCodeProcessor;
import com.ftrend.scan.core.BarCodeScanConfig;
import com.ftrend.scan.inter.ICamera;
...
public class CameraManager implements ICamera {
    ...
    private void checkROIBounds(Rect rect, Point resolution) {
        if (rect.left < 0) {
            rect.left = 0;
        } else if (rect.left > resolution.x) {
            rect.left = resolution.x;
        }
        if (rect.right < rect.left) {
            rect.right = rect.left;
        } else if (rect.right > resolution.x) {
            rect.right = resolution.x;
        }
        if (rect.top < 0) {
            rect.top = 0;
        } else if (rect.top > resolution.y) {
            rect.top = resolution.y;
        }
        if (rect.bottom < rect.top) {
            rect.bottom = rect.top;
        } else if (rect.bottom > resolution.y) {
