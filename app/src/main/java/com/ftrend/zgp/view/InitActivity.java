...
public class InitActivity extends BaseActivity implements InitContract.InitView {
    ...
    TextView mPosCodeTv;
    @BindView(R.id.init_tv_result_title3)
    TextView mDepTitleTv;
...
//                    mPresenter.stopInitData();
            ...
            public void run() {
                try {
                    ...
                    if (!TextUtils.isEmpty(dep)) {
                        ...
                        mDepTv.setVisibility(View.GONE);
                        mDepTitleTv.setVisibility(View.GONE);
                    }
                } catch (Exception e) {
    ...
    public void onBackPressed() {
        if (DataDownloadTask.isRunning()) {
            MessageUtil.info("正在初始化数据，请不要退出");
        } else {
            finish();
        }
    }
