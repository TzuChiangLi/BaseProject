...
//            DbUpdateHelper.update_2(database);
    ...
    @Migration(version = 6, database = ZgpDb.class, priority = 1)
    public static class Migration6_SqbPayOrder extends AlterTableMigration<SqbPayOrder> {

        public Migration6_SqbPayOrder(Class<SqbPayOrder> table) {
            super(table);
        }

        @Override
        public void onPreMigrate() {
            addColumn(SQLiteType.TEXT, "userCode");
        }
    }
    ...
    @Migration(version = 9, database = ZgpDb.class, priority = 1)
    public static class Migration9_TradePay extends AlterTableMigration<TradePay> {

        public Migration9_TradePay(Class<TradePay> table) {
            super(table);
        }

        @Override
        public void onPreMigrate() {
            addColumn(SQLiteType.REAL, "balance");
        }
    }

    @Migration(version = 10, database = ZgpDb.class, priority = 1)
    public static class Migration10 extends BaseMigration {

        @Override
        public void migrate(@NonNull DatabaseWrapper database) {
            DbUpdateHelper.update_10(database);
        }
    }

    @Migration(version = 11, database = ZgpDb.class, priority = 1)
    public static class Migration11 extends BaseMigration {

        @Override
        public void migrate(@NonNull DatabaseWrapper database) {
            DbUpdateHelper.update_11(database);
        }
    }
}
