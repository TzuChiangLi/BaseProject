...
//                Log.e(TAG, "没有需要上传的数据或者单机模式，10秒钟后继续");
            ...
            for (final TradeUploadQueue queue : list) {
                ...
                final String lsNo = queue.getLsNo();
                LogUtil.u(TAG, "流水上传", "上传流水号：" + lsNo);
                ...
                if (trade == null || !trade.getStatus().equals(TradeHelper.TRADE_STATUS_PAID)
                    ...
                    LogUtil.u(TAG, "流水上传", "流水号无效（流水数据不存在或不完整），直接从队列删除：" + lsNo);
                    queue.delete();
                    continue;
                }
                ...
                RestSubscribe.getInstance().uploadTrade(posCode, trade, prodList, pay,
                        new RestCallback(new RestResultHandler() {
                            ...
                            public void onSuccess(RestBodyMap body) {
                                LogUtil.u(TAG, "流水上传", "流水上传成功: " + lsNo);
                                //上传收钱吧交易记录
                                uploadSqb(queue);
                            }
                            ...
                            public void onFailed(String errorCode, String errorMsg) {
                                LogUtil.u(TAG, "流水上传", "流水上传失败: " + errorCode + " - " + errorMsg);
    ...
    private void uploadSqb(final TradeUploadQueue queue) {
        ...
        LogUtil.u(TAG, "收钱吧交易记录上传", "上传收钱吧交易记录: " + queue.getLsNo());
        ...
        for (SqbPayOrder order : orderList) {
            ...
            RestSubscribe.getInstance().uploadSqb(order, result,
                    new RestCallback(new RestResultHandler() {
                        ...
                        @Override
                        public void onFailed(String errorCode, String errorMsg) {
                            LogUtil.u(TAG, "收钱吧交易记录上传", "收钱吧交易记录上传失败: " + errorCode + " - " + errorMsg);
                            isUploading = false;//上传失败不做处理，下次再上传
                        }
                    }));
        }
