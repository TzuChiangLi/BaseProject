    ...
    private static boolean doSaveVipDsc(int index, DatabaseWrapper databaseWrapper) {
        ...
        if (ZgParams.isBhEdition()) {
            ...
            if (!isForceDsc && !prod.isForDsc()) {
                ...
                //优惠率 = 100 - 折扣率
                dscRate = 100 - TradeHelper.vip.getVipDscRate();
                if (dscRate == 100) {
                    //百货版折扣率为0时表示该参数未设置，不计算优惠率
                    dscRate = 0;
                }
            }
        } else {
        ...
        //取会员价和优惠率的较大优惠金额
        double dscByPrice = (price == 0) ? 0 : prod.getPrice() - price;//会员价为0代表未设置会员价，按原价执行
        double dscByRate = prod.getPrice() * dscRate / 100;
        double dsc = Math.max(dscByPrice, dscByRate);
