...
public class RestSubscribe {
     ...
     * 计算积分金额
     *
     * @param trade    流水信息（vipCode，payType，lsNo，tradeFlag）
     * @param prodList 商品列表（prodCode，sortNo，amount，total）
     * @param callback
     */
    public void calcVipTotal(Map<String, Object> trade, List<Map<String, Object>> prodList, final RestCallback callback) {
        RestBodyMap params = new RestBodyMap();
        params.put("trade", trade);
        params.put("prod", prodList);
        RestRequest<RestBodyMap> request = new RestRequest<>();
        request.setBody(params);
        detachAndSubscribe(api.calcVipTotal(request), callback);
    }
     ...
     * @param callback
     */
    public void vipScoreRequest(final String posCode, final String lsNo, final String date,
                                final String cashier, final String vipCode, final String cardCode,
                                final double vipTotal, final double total,
                                final RestCallback callback) {
        RestBodyMap params = new RestBodyMap();
        params.put("posCode", posCode);
        params.put("lsNo", lsNo);
        params.put("date", date);
        params.put("cashier", cashier);
        params.put("vipCode", vipCode);//当前版本可能为空，后台服务需按卡号查询
        params.put("cardCode", cardCode);
        params.put("vipTotal", vipTotal);
        params.put("total", total);
        RestRequest<RestBodyMap> request = new RestRequest<>();
        request.setBody(params);
        detachAndSubscribe(api.vipScoreRequest(request), callback);
    }
     ...
     * @param callback
     */
    public void vipScore(final String dataSign, final RestCallback callback) {
        RestBodyMap params = new RestBodyMap();
        params.put("dataSign", dataSign);
        RestRequest<RestBodyMap> request = new RestRequest<>();
        request.setBody(params);
        detachAndSubscribe(api.vipScore(request), callback);
    }
     ...
     * @param callback
     */
    public void uploadLog(final String posCode, final UserLog log, final RestCallback callback) {
        RestBodyMap params = new RestBodyMap();
        params.put("posCode", posCode);
        params.put("userCode", log.getUserCode());
        params.put("depCode", log.getDepCode());
        params.put("module", log.getModule());
        params.put("function", log.getFunction());
        params.put("occurTime", log.getOccurTime());
        params.put("content", log.getContent());

        RestRequest<RestBodyMap> request = new RestRequest<>();
        request.setBody(params);
        detachAndSubscribe(api.uploadLog(request), callback);
    }
