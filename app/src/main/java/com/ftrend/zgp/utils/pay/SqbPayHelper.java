...
public class SqbPayHelper {
    ...
    public static void activate(final PayResultCallback callback) {
        ...
        if (TextUtils.isEmpty(config.getActivateCode())) {
            //未配置激活码返回失败
            if (callback != null) {
                callback.onResult(false, "", "", "未配置激活码");
            }
            return;
        }
        if (isActivated()) {
        ...
        // 2019/10/18 目前无法获得APPID，激活时无法指定设备信息
    ...
    public static void pay(final Trade trade, final String scanCode, final PayResultCallback callback) {
        ...
        order.setSubject(ZgParams.getCurrentDep().getDepCode().equals("0") ? ZgParams.getCurrentDep().getDepName()
                : ZgParams.getCurrentDep().getDepName() + "-购物消费");//交易简介
