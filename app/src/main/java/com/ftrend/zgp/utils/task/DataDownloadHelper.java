...
public class DataDownloadHelper {
    ...
    private static void savePosDep(final List<RestBodyMap> depList) {
        //插入0专柜是在savePosSysParams方法里保存，为了保存ShopName，暂时放在那里
        //清空数据表
    ...
    private static void savePosSysParams(final List<RestBodyMap> paramsList) {
        ...
        for (RestBodyMap map : paramsList) {
            //TODO 2019年12月21日13:48:49 插入0专柜，后面优化
            if ("ShopName".equals(map.getString("paramName"))) {
                Dep dep = new Dep();
                dep.setDepCode("0");
                dep.setDepName(map.getString("paramValue"));
                dep.save();
            }
