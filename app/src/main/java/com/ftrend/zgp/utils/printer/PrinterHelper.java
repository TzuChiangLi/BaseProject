...
public class PrinterHelper {
     ...
     * 初始化打印机
     */
    public static void initPrinter(final PrintInitCallBack callBack) {
        try {
            if (callBack != null) {
                InnerPrinterManager.getInstance().bindService(App.getContext(), new InnerPrinterCallback() {
                    @Override
                    protected void onConnected(SunmiPrinterService service) {
                        try {
                            ...
                            LogUtil.u("PrinterHelper", "初始化打印机", "初始化失败:" + e.getMessage());
                        }
                    }
     ...
     * @return 是否打印成功
     */
    public static boolean print(int times, List<PrintData> printData) {
        ...
        for (int i = 0; i < times; i++) {
            printDataList.addAll(printData);
        }
        try {
            for (PrintData p : printDataList) {
                ...
                if (p.isTradeList()) {
                    ...
                    for (RestBodyMap data : p.getDataList()) {
                        ...
                        if ("R".equals(reportData.itemName)) {
                        ...
                        if ("T".equals(reportData.itemName)) {
